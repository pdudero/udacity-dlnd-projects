...
class PhysicsSim():
    ...
    def reset(self):
        ...
        self.v = np.zeros(3) if self.init_velocities is None else np.copy(self.init_velocities)
        self.angular_v = np.zeros(3) if self.init_angle_velocities is None else np.copy(self.init_angle_velocities)
        self.linear_accel = np.zeros(3)
        self.angular_accels = np.zeros(3)
        self.prop_wind_speed = np.zeros(4)
        self.thrustplusdrag = np.zeros(3)
        self.crashed = False
        self.done = False
    ...
    def get_linear_forces(self, thrusts):
        ...
        # save thrust+drag for monitoring
        self.thrustplusdrag = np.matmul(body_to_earth_frame(*list(self.pose[3:])), body_forces)
        linear_forces = gravity_force + self.thrustplusdrag
    ...
    def get_propeler_thrust(self, rotor_speeds):
        ...
        for prop_number in range(4):
            V = self.prop_wind_speed[prop_number]     # freestream fluid speed
            D = self.propeller_size                   # propeller diameter
            n = rotor_speeds[prop_number]             # cycles/second
            J = V / n * D                             # advance ratio
            # From http://m-selig.ae.illinois.edu/pubs/BrandtSelig-2011-AIAA-2011-1255-LRN-Propellers.pdf
            C_T = max(.12 - .07*max(0, J)-.1*max(0, J)**2, 0)   # Thrust coefficient
    ...
    def next_timestep(self, rotor_speeds):
        ...
        forces = self.get_linear_forces(thrusts)
        ...
        if self.crashed:
            ...
            self.prop_wind_speed = np.zeros(4)
            self.thrustplusdrag = np.zeros(3)
        elif self.pose[2] > 0 or forces[2] > 0:
            self.linear_accel = forces / self.mass
