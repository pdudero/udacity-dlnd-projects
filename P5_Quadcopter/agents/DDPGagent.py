import numpy as np
#from agents.critic_128x256 import Critic
#from agents.actor_128x256x128 import Actor
from agents.critic_orig import Critic
from agents.actor_orig import Actor
...
class DDPG():
    ...
    def step(self, 
             action,     # list length action_size
    ...
    def act(self, state):
        ...
        state = np.reshape(state, [-1, self.state_size]) # batch dim first
        action = self.actor_local.model.predict(state)[0]
        noisesample = self.action_range*self.noise.sample()  # add some noise for exploration
        self.noisevar += noisesample**2
        action = np.clip([action+noisesample],self.action_low,self.action_high)[0]
        return list(action)
